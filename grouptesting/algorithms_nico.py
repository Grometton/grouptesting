...
def D_log(_eps):
    D = (1+_eps)*np.log(1+_eps) - _eps
    return D

def PD_hat(X, y, delta, nu, num_tests, num_defectives):
    sigma_hat = np.zeros(shape=(X.shape[0],)) # Generate the sigma hat matrix
    # find set of possibly defective items 
    neg_tests = np.where(y < 1)[0]
    N_neg_j = np.sum(X[:, neg_tests], axis=1)
    possibly_defectives = np.where(N_neg_j < (delta * num_tests * nu / num_defectives))[0]
    return possibly_defectives


def NDD(X, y, delta, nu, num_tests, num_defectives):
  ...
  neg_tests = np.where(y < 1)[0]
  N_neg_j = np.sum(X[:, neg_tests], axis=1)
  # ND = np.where(N_neg_j >= delta * n * nu / k)[0]

  # # Emulate NCOMP:
  # return (N_neg_j < (delta * num_tests * nu / num_defectives))
  ...
  #print(pos_tests_with_one_defective)
  onedefective = np.where(X[possibly_defectives][:, pos_tests[pos_tests_with_one_defective]] == 1)[0]
